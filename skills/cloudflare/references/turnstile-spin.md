...
You are the agent. Run the wizard below by invoking the scripts under `scripts/turnstile-spin/` and branching on their JSON output. The scripts hold the deterministic logic (API calls, retry/error handling); your job is orchestration, codebase reading, confirmation, and the frontend + backend edits.
...
| Frontend | Reference |
|---|---|
| Vanilla HTML | [vanilla-html](turnstile-spin/vanilla-html.md) |
| Next.js App Router | [nextjs-app](turnstile-spin/nextjs-app.md) |
| Next.js Pages Router | [nextjs-pages](turnstile-spin/nextjs-pages.md) |
| Astro | [astro](turnstile-spin/astro.md) |
| SvelteKit | [sveltekit](turnstile-spin/sveltekit.md) |
| Hugo | [hugo](turnstile-spin/hugo.md) |
...
3. **Auth + scope probe (FIRST irreversible action).** Run `scripts/turnstile-spin/auth-probe.sh`. If account enumeration needs Wrangler, set `PROJECT_ROOT`, approved canonical `WRANGLER_BIN`, and exact `WRANGLER_VERSION` first. Branch on `status`:
...
10. **Validation.** For a newly created widget, set `EXPECTED_DOMAINS_JSON` to the user-approved JSON array and run `(set +x; printf '%s' "$WIDGET_SECRET" | scripts/turnstile-spin/validate.sh --sitekey "$SITEKEY" --account-id "$ACCOUNT_ID" --expected-domains "$EXPECTED_DOMAINS_JSON")`, then unset `WIDGET_SECRET`. The validator reads the secret only from standard input and never writes it to disk or command arguments. For an existing widget, the guarded flow validates the retrieved secret before storing it. In both flows, exercise the actual protected backend with a fresh real Turnstile token, verify one successful request, then verify that replaying the token is rejected. If the backend cannot be run, report destination validation as pending and do not claim end-to-end success. **[wait for user if anything fails]**

11. **Final report.** Print the structured summary: what was created, what was validated, what to do next.
...
| Multiple Cloudflare accounts                   | `scripts/turnstile-spin/auth-probe.sh` returns all accounts; ask the user to choose, export `CLOUDFLARE_ACCOUNT_ID`                                                                                                                                  |
...
| Token expired mid-flow                         | Stop, re-run `scripts/turnstile-spin/auth-probe.sh`, prompt for fresh credentials                                                                                                                                                                    |
