...
1. **Domain onboarded?** Run `npx wrangler email sending list` to see which domains have email sending enabled. If the domain isn't listed, run `npx wrangler email sending enable userdomain.com` or see [cli-and-mcp.md](cloudflare-email-service/cli-and-mcp.md) for full setup instructions.
...
|--------------|------|-----------|
| **Send emails from a Cloudflare Worker** | Workers binding (no API keys needed) | [sending.md](cloudflare-email-service/sending.md) |
| **Send emails from an AI agent built with [Cloudflare Agents SDK](https://developers.cloudflare.com/agents/)** | `onEmail()` + `replyToEmail()` in Agent class | [sending.md](cloudflare-email-service/sending.md) |
| **Send emails from an external app or agent** (Node.js, Go, Python, etc.) | REST API with Bearer token | [rest-api.md](cloudflare-email-service/rest-api.md) |
| **Send emails from a coding agent** (Claude Code, Cursor, Copilot, etc.) | MCP tools, wrangler CLI, or REST API | [cli-and-mcp.md](cloudflare-email-service/cli-and-mcp.md) |
| **Receive and process incoming emails** (Email Routing) | Workers `email()` handler | [routing.md](cloudflare-email-service/routing.md) |
| **Set up Email Sending or Email Routing** | `wrangler email sending enable` / `wrangler email routing enable`, or Dashboard | [cli-and-mcp.md](cloudflare-email-service/cli-and-mcp.md) |
| **Improve deliverability, avoid spam folders** | Authentication, content, compliance | [deliverability.md](cloudflare-email-service/deliverability.md) |

## Sending Workflow

Prefer the binding for Workers; use REST for external apps or when explicitly requested. Read [sending.md](cloudflare-email-service/sending.md) or [rest-api.md](cloudflare-email-service/rest-api.md) to retrieve the documentation for the selected task before writing code. These guides cover setup, recipients, attachments, headers, limits, response handling, and errors; the Workers guide also covers Agents SDK integration and types matched to the project configuration.
...
- **[cloudflare-email-service/sending.md](cloudflare-email-service/sending.md)** — Documentation map for Workers binding, attachments, and Agents SDK email.
- **[cloudflare-email-service/rest-api.md](cloudflare-email-service/rest-api.md)** — Documentation map for HTTP sending, request schemas, responses, and errors.
- **[cloudflare-email-service/routing.md](cloudflare-email-service/routing.md)** — Inbound `email()` handler, forwarding, replying, parsing. For receiving emails.
- **[cloudflare-email-service/cli-and-mcp.md](cloudflare-email-service/cli-and-mcp.md)** — Domain setup, wrangler commands, MCP tools. For first-time setup.
- **[cloudflare-email-service/deliverability.md](cloudflare-email-service/deliverability.md)** — SPF/DKIM/DMARC, bounces, suppressions, best practices.
