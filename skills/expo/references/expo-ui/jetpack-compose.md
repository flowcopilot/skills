...
- **Before writing any code, run the list-components script** to get the exact components and modifiers available in the installed version:
  ```bash
  node <skill-root>/scripts/expo-ui/list-components.js <project-path>          # names only (compact)
  node <skill-root>/scripts/expo-ui/list-components.js <project-path> --docs   # with one-line descriptions
  ```
