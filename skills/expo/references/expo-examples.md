...
Reach for an example before hand-rolling an integration. (Kinds — full-stack, showcases, starters — are noted in `expo-examples/catalog.md`.)
...
Map the user's need to an example name (e.g. payments → `with-stripe`, auth → `with-clerk`). `expo-examples/catalog.md` is a categorized snapshot for fast triage — but it drifts, so confirm against the live list:
...
- `expo-examples/catalog.md` — categorized snapshot of the example library for fast triage.
