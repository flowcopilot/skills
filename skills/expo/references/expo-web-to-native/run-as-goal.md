...
Disclosed reference for [`expo-web-to-native`](../expo-web-to-native.md) - the recommended way to drive the whole migration. The migration is a repeat-until-done loop (assess → nativize each screen → verify → check off), which is exactly the shape of a **goal loop**: a single objective re-injected every turn until the worklist is empty. This file carries a ready-shaped, migration-specific objective - and, crucially, the objective **re-reads this skill every iteration**, so the loop keeps following the playbook (and self-bootstraps the assess step if no worklist exists yet).
...
Each iteration, FIRST re-read the playbook - references/expo-web-to-native.md (and its references) - then:
