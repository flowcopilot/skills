...
- **SwiftUI/UIKit app with no React Native runtime:** read [eas-app-stores/native-ios.md](eas-app-stores/native-ios.md) before changing its build configuration. Keep the Xcode project and Swift code as the app's source of truth. The Expo/React Native quick-start and development-client examples below do not apply to this path.
- **Native Android app with no React Native runtime:** preserve its existing native build setup. See [eas-app-stores/play-store.md](eas-app-stores/play-store.md) for submission; the Swift/iOS setup instructions do not apply. A dedicated native Android setup walkthrough is not yet included.
...
Use the archive and icon checks in [eas-app-stores/native-ios.md](eas-app-stores/native-ios.md) during initial native iOS setup, after changing versioning, bundle IDs, or icons, or when diagnosing a rejected upload. Routine releases follow the EAS build/submit flow and the processing and availability checks in [eas-app-stores/testflight.md](eas-app-stores/testflight.md). A successful EAS build or a queued submission does not establish Apple acceptance, tester access, or an App Store release.

## References

Consult these resources as needed:

- eas-app-stores/workflows.md -- CI/CD workflows for automated store releases and PR previews
- eas-app-stores/testflight.md -- Submitting iOS builds to TestFlight for beta testing
- eas-app-stores/app-store-metadata.md -- Managing App Store metadata and ASO optimization
- eas-app-stores/play-store.md -- Submitting Android builds to Google Play Store
- eas-app-stores/ios-app-store.md -- iOS App Store submission and review process
- eas-app-stores/native-ios.md -- Native Swift/SwiftUI/UIKit setup, versioning, and archive verification without an Expo runtime
...
Example for an Expo / React Native project (native Swift profiles are in `eas-app-stores/native-ios.md`):
...
- For native Swift apps, use the explicit build/submit flow in `eas-app-stores/native-ios.md`
- For Expo / React Native apps, `npx testflight` provides a quick TestFlight flow
- Configure Apple credentials via `eas credentials`
- See eas-app-stores/testflight.md for credential setup
- See eas-app-stores/ios-app-store.md for App Store submission

### Android

- Set up Google Play Console service account
- Configure tracks: internal → closed → open → production
- See eas-app-stores/play-store.md for detailed setup

## Automated Releases

EAS Workflows automate the build → submit → update pipeline for CI/CD. See eas-app-stores/workflows.md for store-release examples. To author or validate workflow YAML, use the `eas-workflows` skill - it works from the live workflow schema.
