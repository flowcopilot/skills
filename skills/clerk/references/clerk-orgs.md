...
> **Requirements:** Requires NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY and CLERK_SECRET_KEY. Organizations must be enabled in Clerk Dashboard → Organizations. Membership mode (required vs optional) must match the B2B vs B2C + B2B coexistence story of your app.
...
>
> **Version**: This skill targets current SDKs (`@clerk/nextjs` v7+, `@clerk/react` v6+ — Core 3). Core 2 differences are noted inline with `> **Core 2 ONLY (skip if current SDK):**` callouts — see [Clerk router](../SKILL.md) for the full version table.
...
5. **Cap membership** — set `maxAllowedMemberships` at org creation or pick a seat-limited Billing Plan (see [clerk-billing](clerk-billing.md) skill).

## What Do You Need?

| Task | Reference |
|------|-----------|
| System permissions catalog, custom roles, role sets | clerk-orgs/roles-permissions.md |
| Invitation lifecycle (create, list, revoke, built-in UI) | clerk-orgs/invitations.md |
| Enterprise SSO setup, provider field access, domain verification | clerk-orgs/enterprise-sso.md |
| Next.js adaptations for orgs (role/permission middleware, slug invariants, orgId-scoped writes) | clerk-orgs/nextjs-patterns.md |

## References

| Reference | Description |
|-----------|-------------|
| `clerk-orgs/roles-permissions.md` | Default + custom roles, System Permissions catalog, permission naming |
| `clerk-orgs/invitations.md` | Backend API for invitations + built-in UI |
| `clerk-orgs/enterprise-sso.md` | SAML/OIDC per-org, domain verification, correct field access |
| `clerk-orgs/nextjs-patterns.md` | Next.js adaptations specific to orgs. For generic Next.js patterns see `clerk-nextjs-patterns` skill. |
...
Pre-req: a linked project (`clerk auth login` + `clerk link`, see [clerk-cli](clerk-cli.md)) — or an unclaimed app from `clerk init`: `clerk enable orgs` and org CRUD via `clerk api` work with no login at all.
...
- This handles **org config + CRUD**. Subscription / billing for orgs (org plans, seat-limit pricing) flows through [clerk-billing](clerk-billing.md) skill.
- Roles + permissions catalog is editable in `clerk-orgs/roles-permissions.md`. Custom role creation goes through `clerk config patch` (instance-level role definitions) — see Dashboard's role editor for the UX equivalent.
- For SSO / verified domain provisioning, see `clerk-orgs/enterprise-sso.md`.
...
**Permission naming convention.** System Permissions prefix with `org:sys_`; custom Permissions use `org:<resource>:<action>`. The full System Permissions catalog lives in `clerk-orgs/roles-permissions.md` — the short list is:
...
Do NOT invent names like `org:create`, `org:manage_members`, `org:update_metadata` — those are not real permission slugs. See `clerk-orgs/roles-permissions.md` for custom roles and the permission table.
...
You can create up to 10 custom roles per instance in Dashboard → Organizations → Roles & Permissions. Role-per-org is controlled via **Role Sets** — see `clerk-orgs/roles-permissions.md` for the full model (custom roles, Creator/Default role settings, role sets, and the System Permissions catalog).
...
For tier-based seat limits tied to a subscription, use a seat-limited Billing Plan (see [clerk-billing](clerk-billing.md)).
...
For middleware-level protection (Next.js) see `clerk-orgs/nextjs-patterns.md`.
...
The full lifecycle (list, revoke, bulk create, built-in `<OrganizationProfile />` UI) lives in `clerk-orgs/invitations.md`.
...
- [clerk-billing](clerk-billing.md) — Seat-limit plans, per-plan billing, `has({ plan })` / `has({ feature })`
- [clerk-webhooks](clerk-webhooks.md) — Sync org events to your database (`organization.created`, `organizationMembership.*`)
- [clerk-backend-api](clerk-backend-api.md) — Full Backend API reference
