...
|------------------------|------|-----------|
| "Add auth to my app" / "add sign-in with Clerk" | Prebuilt native components (default) | clerk-expo/setup.md + clerk-expo/prebuilt-components.md |
| "Add auth" but Expo Go / web / custom UI required | Custom flows | clerk-expo/setup.md + clerk-expo/custom-flows.md |
| "Add phone / SMS auth", "email OTP", "passwordless" | Custom flow, `phoneCode` / `emailCode` | clerk-expo/custom-flows.md |
| "Sign in with Google/Apple/GitHub", "social login", "SSO" | Browser SSO or native buttons | clerk-expo/sso-and-native-auth.md |
| "MFA / 2FA / TOTP", "forgot password", "email link" | Custom flow additions | clerk-expo/custom-flows.md |
| "Protect routes/screens", "redirect if signed out" | Expo Router guards | clerk-expo/protected-routes.md |
| "Show user profile", "org switching", "push notifications", "sign out", "call my backend" | App recipes | clerk-expo/recipes.md |
| "Biometric login", "Face ID", "passkeys" | Device features | clerk-expo/recipes.md |
...
2. Follow clerk-expo/setup.md: install, env key, provider, token cache, config plugin, build type.
...
- The config plugin accepts a `theme` JSON file for native component styling (see clerk-expo/prebuilt-components.md).
...
- [clerk-orgs](clerk-orgs.md), [clerk-billing](clerk-billing.md), [clerk-webhooks](clerk-webhooks.md) — feature skills (hooks work the same in Expo)
