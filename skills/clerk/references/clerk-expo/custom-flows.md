...
1. **Check enabled factors** ([clerk-expo](../clerk-expo.md) Gate 3). Derive the Frontend API URL from the publishable key and fetch `<frontendApiUrl>/v1/environment?_is_native=true`, or have the developer confirm in the dashboard. Only implement enabled strategies; if the user asked for a disabled one (common with SMS), tell them to enable it in Clerk Dashboard → **User & authentication** first.
