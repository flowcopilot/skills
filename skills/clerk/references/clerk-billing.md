...
>
> **Note**: Billing APIs are still experimental. Pin your `@clerk/nextjs` and `clerk-js` package versions. See [Clerk router](../SKILL.md) for the supported version table.
...
Pre-req: project linked to the Clerk app (`clerk auth login` + `clerk link`, see [clerk-cli](clerk-cli.md)). Billing is account-only — unlike orgs, it cannot be enabled on an unclaimed app; claim the app first.
...
- This handles **billing config** (toggles + plans + features catalog). **Subscription lifecycle** (users picking a plan, checkout, renewal, cancellation) still flows through `<PricingTable />` + billing webhooks, see [clerk-webhooks](clerk-webhooks.md) skill for the lifecycle events.
...
| Task | Reference |
|------|-----------|
| `<PricingTable />` props, `<CheckoutButton />`, `<Show>` billing patterns | clerk-billing/billing-components.md |
| B2C patterns (individual user subscriptions, `Membership optional` prerequisite) | clerk-billing/b2c-patterns.md |
| B2B patterns (org subscriptions, seat-limit plans, admin-gated billing UI) | clerk-billing/b2b-patterns.md |
| Webhook event catalog, payload shapes, handler templates | clerk-billing/billing-webhooks.md |

## References

| Reference | Description |
|-----------|-------------|
| `clerk-billing/billing-components.md` | `<PricingTable />` and subscription UI |
| `clerk-billing/b2c-patterns.md` | B2C subscription billing patterns |
| `clerk-billing/b2b-patterns.md` | B2B billing with organization subscriptions and seat-limit plans |
| `clerk-billing/billing-webhooks.md` | Subscription lifecycle event handling |
...
See `clerk-billing/b2b-patterns.md` for tiered plan naming, seat-limit invariants, admin-only billing, and webhook handlers.
...
For the full template covering all 15 events, the TS type declarations from `@clerk/backend`, the `proxy.ts` public-route setup, and the subscription status value table, see `clerk-billing/billing-webhooks.md`.
...
- [clerk-orgs](clerk-orgs.md) - B2B organizations (required for B2B billing and seat-limit plans)
- [clerk-webhooks](clerk-webhooks.md) - Webhook signature verification and routing
