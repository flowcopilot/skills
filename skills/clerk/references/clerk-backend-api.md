...
> **Requirements:** Requires CLERK_SECRET_KEY (sk_*) for Backend API calls.

## Options context

User Prompt: the user's request that selected this reference.
...
**ALWAYS execute requests with direct `curl` commands.** Use the spec-extraction scripts (`api-specs-context.sh`, `extract-tags.js`, `extract-endpoint-detail.sh`) to discover endpoints, but make actual API calls with `curl`. Do NOT use `scripts/clerk-backend-api/execute-request.sh` — it's a local dev helper, not for agent use.
...
```bash
bash scripts/clerk-backend-api/api-specs-context.sh
```
...
- Use direct curl commands for all API calls — never use `scripts/clerk-backend-api/execute-request.sh`.
...
```bash
curl -s https://raw.githubusercontent.com/clerk/openapi-specs/main/bapi/${version_name} | node scripts/clerk-backend-api/extract-tags.js
```
...
```bash
curl -s https://raw.githubusercontent.com/clerk/openapi-specs/main/bapi/${version_name} | bash scripts/clerk-backend-api/extract-tag-endpoints.sh "${tag_name}"
```
...
```bash
curl -s https://raw.githubusercontent.com/clerk/openapi-specs/main/bapi/${version_name} | bash scripts/clerk-backend-api/extract-endpoint-detail.sh "${path}" "${method}"
```
...
4. Build and execute a **direct curl command** (see How to execute requests above). Do NOT use `scripts/clerk-backend-api/execute-request.sh`.
...
- [clerk-orgs](clerk-orgs.md) - Manage organizations via API
- [clerk-webhooks](clerk-webhooks.md) - Real-time event sync
