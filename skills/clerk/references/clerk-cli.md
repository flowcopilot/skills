...
For setup or `init` requests, use the [`clerk-setup`](clerk-setup.md) skill as the source of truth. Its package-runner, accountless, migration, and init-first guidance takes precedence over the general operational prerequisites below.
...
Except when following [clerk-setup](clerk-setup.md), verify the CLI is authenticated, linked, and healthy before running other Clerk commands in a session:
...
Which commands need an account, and the full rules: [auth.md](clerk-cli/auth.md#accountless-operating-without-an-account).
...
A project is "linked" to an application via `clerk link`. Once linked, most commands auto-resolve the target app and dev instance from the repo's git remote. To target something else, pass `--app <id>` and/or `--instance dev|prod|<instance_id>`. See [clerk-cli/auth.md](clerk-cli/auth.md) for the full resolution order.
...
See [clerk-cli/recipes.md](clerk-cli/recipes.md) for concrete patterns: listing/filtering users, creating orgs, impersonation sessions, etc.
...
| `clerk webhooks listen`       | First-party local webhook tunnel (like `stripe listen`): opens a Svix relay inbox URL and forwards each delivery to your local handler. No auth, no linked project, no Clerk API. Full flow in [clerk-cli/recipes.md](clerk-cli/recipes.md#webhooks-local-testing).                                                 | `--forward-to <url>` (required), `--token <c_token>`, `-H, --header <k:v>` (repeatable), `--json` (NDJSON)                                                                       |
...
| `clerk enable orgs` / `clerk disable orgs` | Toggle Organizations on the instance. For org features, components, and API usage, see the [clerk-orgs](clerk-orgs.md) skill.                                                                                                                                                                                         | `--force-selection`, `--auto-create`, `--max-members <n>`, `--domains`, `--dry-run`, `--yes`, `--app`, `--instance`                                                              |
| `clerk enable billing` / `clerk disable billing` | Toggle billing for users and/or orgs (defaults to both). For plans, pricing components, and entitlements, see the [clerk-billing](clerk-billing.md) skill.                                                                                                                                                         | `--for <orgs\|users>`, `--dry-run`, `--yes`, `--no-skills` (enable only), `--app`, `--instance`                                                                                  |
...
- **For `init`, follow [clerk-setup](clerk-setup.md).** It owns framework-support, accountless, app-linking, migration, and login-order guidance so setup instructions have one source of truth.
...
- [clerk-cli/auth.md](clerk-cli/auth.md) - auth flow, key resolution order, host-vs-sandbox behavior, `--app`/`--instance` targeting, Backend vs Platform API.
- [clerk-cli/recipes.md](clerk-cli/recipes.md) - copy-pasteable recipes for common Clerk tasks.
- [clerk-cli/agent-mode.md](clerk-cli/agent-mode.md) - agent-mode behavior matrix, sandbox warning semantics, exit codes, error format.
