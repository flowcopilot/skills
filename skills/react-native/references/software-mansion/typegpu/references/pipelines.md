...
> WebGPU clip space uses `z in [0, 1]`, not OpenGL's `[-1, 1]`. A copy-pasted GL perspective matrix will clip the near plane. With `depthClearValue: 1.0` + `depthCompare: 'less'`, near is `z = 0`. Reversed-Z: `depthClearValue: 0.0` + `depthCompare: 'greater'`, near at `z = 1`. See "WebGPU Coordinate Conventions" in index.md.
...
> `albedoView`/`normalView`/`positionView` are cached at setup - see "Cache bind groups and views" in index.md.
