...
Catalog semantics (updates replace, stream end = offline, playable lists) are in index.md. Video-track playability on iOS is based on codec families the renderer recognizes; actual decode support is still decided by AVFoundation at runtime.
...
Channel semantics are in index.md; the iOS wiring:
