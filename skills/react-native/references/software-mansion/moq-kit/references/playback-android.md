...
Catalog semantics (updates replace, stream end = offline, playable lists) are in index.md. Wrap the collect in `try`/`catch`: the stream usually ends by throwing `MoqException$Mux` when the producer goes away, so the offline path needs to run from both `catch` and normal completion — see `observeCatalogs` in the demo's `PlayerDemoViewModel`.
...
Channel semantics are in index.md; the Android wiring:
