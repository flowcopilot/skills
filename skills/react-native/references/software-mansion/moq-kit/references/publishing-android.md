...
Shared ordering and publisher states are in index.md. Android specifics: `Publisher()` doesn't throw; `start()`/`stop()` are **not** suspend; `start()` validates every track's codec first and throws `UnsupportedCodecException` before starting anything; `start()` with zero tracks flips straight to `Stopped` (as does stopping the last track).
